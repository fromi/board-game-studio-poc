...
const PlayedCards = ({game, playerId, undo}) => {
  ...
  if (playerId === CREATURE) {
    ...
    if (hunted.playedPlaceCards.length === 1) {
      tooltip = t('You played {{place}}', {place: t(places[hunted.playedPlaceCards[0].place].name)})
    }
    return (
      <Tooltip title={tooltip} enterTouchDelay={0}>
        <div className="played-cards">
          {hunted.playedPlaceCards.map(({place, revealed}) => (
            <PlaceCard place={place} faceDown={!revealed} key={place} onClick={() => undo({type: PLAY_PLACE_CARD, place})}/>
          ))}
        </div>
      </Tooltip>
    )
